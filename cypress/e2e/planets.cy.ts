describe("template spec", () => {
  const visit = () => {
    ...
    const planetsTab = cy
      .get('[data-testid="tabs-menu-label"]')
      .contains("planets");
    planetsTab.click();
  };
  ...
  it("show elements when an existing filter is applied", () => {
    ...
    // We assume that there will always be a planet with "a" in its name.
  ...
  it("show elements when a non-existing filter is applied", () => {
    ...
    // We assume that there won't be a planet with "<random-filter>" in its
    // name.
    const filterInput = cy.get('[data-testid="filters-input"]');
